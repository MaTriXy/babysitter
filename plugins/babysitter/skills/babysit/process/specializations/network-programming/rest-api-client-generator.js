...
export const specParserTask = defineTask('spec-parser', (args, taskCtx) => ({
  ...
  agent: { name: 'parser-engineer', prompt: { role: 'OpenAPI Parser Engineer', task: 'Implement spec parser', context: args, instructions: ['1. Parse OpenAPI spec', '2. Extract endpoints', '3. Extract schemas', '4. Handle references'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const clientGeneratorTask = defineTask('client-generator', (args, taskCtx) => ({
  ...
  agent: { name: 'client-engineer', prompt: { role: 'Client Code Generator', task: 'Generate API client', context: args, instructions: ['1. Generate client class', '2. Generate endpoint methods', '3. Handle parameters', '4. Generate async methods'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const errorHandlingTask = defineTask('error-handling', (args, taskCtx) => ({
  ...
  agent: { name: 'error-engineer', prompt: { role: 'Error Handling Engineer', task: 'Implement error handling', context: args, instructions: ['1. Define error types', '2. Parse error responses', '3. Type-safe errors', '4. Error recovery'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const authenticationTask = defineTask('authentication', (args, taskCtx) => ({
  ...
  agent: { name: 'auth-engineer', prompt: { role: 'Auth Generator Engineer', task: 'Generate auth support', context: args, instructions: ['1. API key auth', '2. Bearer auth', '3. OAuth2 flows', '4. Custom auth'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const documentationTask = defineTask('documentation', (args, taskCtx) => ({
  ...
  agent: { name: 'doc-engineer', prompt: { role: 'Documentation Engineer', task: 'Generate client documentation', context: args, instructions: ['1. Generate README', '2. Generate API docs', '3. Generate examples', '4. Generate changelog'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const validationTask = defineTask('validation', (args, taskCtx) => ({
  ...
  agent: { name: 'qa-engineer', prompt: { role: 'QA Engineer', task: 'Validate generator', context: args, instructions: ['1. Verify spec compliance', '2. Check generated code', '3. Validate tests', '4. Calculate score'] }, outputSchema: { type: 'object', required: ['overallScore', 'passedChecks', 'artifacts'], properties: { overallScore: { type: 'number' }, passedChecks: { type: 'array' }, artifacts: { type: 'array' } } } },
  io: { inputJsonPath: `tasks/${taskCtx.effectId}/input.json`, outputJsonPath: `tasks/${taskCtx.effectId}/result.json` }, labels: ['network', 'api-generator', 'validation']
}));
