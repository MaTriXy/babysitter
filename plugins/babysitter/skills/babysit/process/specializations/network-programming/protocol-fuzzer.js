...
export const architectureTask = defineTask('architecture', (args, taskCtx) => ({
  ...
  agent: { name: 'architect', prompt: { role: 'Systems Architect', task: 'Design protocol fuzzer', context: args, instructions: ['1. Fuzzing loop design', '2. Input queue', '3. Mutation pipeline', '4. Result analysis'] }, outputSchema: { type: 'object', required: ['design', 'artifacts'], properties: { design: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const protocolModelTask = defineTask('protocol-model', (args, taskCtx) => ({
  ...
  agent: { name: 'model-engineer', prompt: { role: 'Protocol Model Engineer', task: 'Build protocol model', context: args, instructions: ['1. Message structure', '2. Field definitions', '3. State machine', '4. Constraints'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const generationEngineTask = defineTask('generation-engine', (args, taskCtx) => ({
  ...
  agent: { name: 'generation-engineer', prompt: { role: 'Generation Engineer', task: 'Build generation engine', context: args, instructions: ['1. Grammar-based generation', '2. Random generation', '3. Edge case generation', '4. Sequence generation'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const transportLayerTask = defineTask('transport-layer', (args, taskCtx) => ({
  ...
  agent: { name: 'transport-engineer', prompt: { role: 'Transport Engineer', task: 'Build transport layer', context: args, instructions: ['1. TCP transport', '2. UDP transport', '3. TLS transport', '4. Raw socket transport'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const coverageTrackingTask = defineTask('coverage-tracking', (args, taskCtx) => ({
  ...
  agent: { name: 'coverage-engineer', prompt: { role: 'Coverage Engineer', task: 'Implement coverage tracking', context: args, instructions: ['1. Code coverage', '2. State coverage', '3. Path coverage', '4. Coverage-guided mutation'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const validationTask = defineTask('validation', (args, taskCtx) => ({
  ...
  agent: { name: 'qa-engineer', prompt: { role: 'QA Engineer', task: 'Validate fuzzer', context: args, instructions: ['1. Verify engines', '2. Check detection', '3. Validate tests', '4. Calculate score'] }, outputSchema: { type: 'object', required: ['overallScore', 'passedChecks', 'artifacts'], properties: { overallScore: { type: 'number' }, passedChecks: { type: 'array' }, artifacts: { type: 'array' } } } },
