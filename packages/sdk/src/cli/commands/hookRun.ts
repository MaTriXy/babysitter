  ...
  switch (hookType) {
    ...
    default: {
      const error = {
        ...
        message: `Unknown hook type: ${hookType}. Supported: stop, session-start, user-prompt-submit, pre-tool-use`,
      };
      if (json) {
        process.stderr.write(JSON.stringify(error, null, 2) + "\n");
      } else {
        process.stderr.write(
          `Error: Unknown hook type: ${hookType}. Supported: stop, session-start, user-prompt-submit, pre-tool-use\n`,
        );
      }
      return 1;
    }
