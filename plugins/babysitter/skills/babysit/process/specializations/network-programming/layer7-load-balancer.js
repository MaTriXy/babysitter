...
export const architectureTask = defineTask('architecture', (args, taskCtx) => ({
  ...
  agent: { name: 'architect', prompt: { role: 'Systems Architect', task: 'Design L7 LB architecture', context: args, instructions: ['1. HTTP processing pipeline', '2. Connection pooling', '3. SSL/TLS handling', '4. Routing engine design'] }, outputSchema: { type: 'object', required: ['design', 'artifacts'], properties: { design: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const httpParsingTask = defineTask('http-parsing', (args, taskCtx) => ({
  ...
  agent: { name: 'http-engineer', prompt: { role: 'HTTP Engineer', task: 'Implement HTTP parsing', context: args, instructions: ['1. Request parsing', '2. Response parsing', '3. Chunked encoding', '4. HTTP/2 support'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const sslTerminationTask = defineTask('ssl-termination', (args, taskCtx) => ({
  ...
  agent: { name: 'ssl-engineer', prompt: { role: 'SSL Engineer', task: 'Implement SSL termination', context: args, instructions: ['1. Certificate management', '2. SNI support', '3. OCSP stapling', '4. TLS 1.3 support'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const headerManipulationTask = defineTask('header-manipulation', (args, taskCtx) => ({
  ...
  agent: { name: 'header-engineer', prompt: { role: 'Header Engineer', task: 'Implement header manipulation', context: args, instructions: ['1. Add/remove headers', '2. X-Forwarded headers', '3. CORS handling', '4. Security headers'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const sessionAffinityTask = defineTask('session-affinity', (args, taskCtx) => ({
  ...
  agent: { name: 'session-engineer', prompt: { role: 'Session Engineer', task: 'Implement session affinity', context: args, instructions: ['1. Cookie-based affinity', '2. IP hash affinity', '3. Header-based affinity', '4. Sticky sessions'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const healthCheckingTask = defineTask('health-checking', (args, taskCtx) => ({
  ...
  agent: { name: 'health-engineer', prompt: { role: 'Health Check Engineer', task: 'Implement health checks', context: args, instructions: ['1. HTTP health checks', '2. Custom health endpoints', '3. Active/passive checks', '4. Circuit breaker'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const validationTask = defineTask('validation', (args, taskCtx) => ({
  ...
  agent: { name: 'qa-engineer', prompt: { role: 'QA Engineer', task: 'Validate load balancer', context: args, instructions: ['1. Verify routing', '2. Check SSL', '3. Validate tests', '4. Calculate score'] }, outputSchema: { type: 'object', required: ['overallScore', 'passedChecks', 'artifacts'], properties: { overallScore: { type: 'number' }, passedChecks: { type: 'array' }, artifacts: { type: 'array' } } } },
