#!/bin/bash
# Cleanup browser runtime/container state after successful run completion.
...
if [[ -z "$RUN_ID" ]]; then
  echo "[on-run-complete/browser-cleanup] Missing runId; skipping" >&2
  exit 0
fi

if [[ "$(brt_bool "${BABYSITTER_BROWSER_PRESERVE_RUNTIME:-false}")" == "true" ]]; then
  echo "[on-run-complete/browser-cleanup] Preserving browser runtime state for run $RUN_ID" >&2
  exit 0
fi
...
if [[ ! -f "$STATE_FILE" ]]; then
  echo "[on-run-complete/browser-cleanup] No browser runtime state for run $RUN_ID" >&2
  exit 0
fi
...
echo "[on-run-complete/browser-cleanup] Removed browser runtime state for run $RUN_ID" >&2
exit 0
