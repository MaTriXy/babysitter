...
suite('PTY stdin controls', () => {
  test('sends ESC and Enter via PTY stdin', async () => {
    ...
    const proc = spawnPtyProcess(process.execPath, [scriptPath], { cwd: tempDir });
    let output = '';
    ...
    try {
      ...
      assert.strictEqual(exit.exitCode, 0, `expected exit code 0, got ${exit.exitCode} (output=${output})`);

      const match = output.match(/BYTES=([^\r\n]+)/);
      assert.ok(match?.[1], `expected BYTES marker in output: ${output}`);
