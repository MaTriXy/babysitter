...
export const planAnalysisTask = defineTask('plan-analysis', (args, taskCtx) => ({
  ...
  agent: { name: 'general-purpose', prompt: { role: 'Strategic analyst', task: 'Analyze the plan structure and assumptions', context: args, instructions: ['Document plan objectives', 'List key milestones', 'Identify critical dependencies', 'List explicit assumptions', 'Identify implicit assumptions'], outputFormat: 'JSON with objectives, milestones, dependencies, explicitAssumptions, implicitAssumptions, artifacts' }, outputSchema: { type: 'object', required: ['objectives', 'dependencies', 'artifacts'], properties: { objectives: { type: 'array' }, milestones: { type: 'array' }, dependencies: { type: 'array' }, explicitAssumptions: { type: 'array' }, implicitAssumptions: { type: 'array' }, artifacts: { type: 'array' } } } },
