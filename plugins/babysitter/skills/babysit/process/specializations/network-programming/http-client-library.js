...
export const requestBuilderTask = defineTask('request-builder', (args, taskCtx) => ({
  ...
  agent: { name: 'request-engineer', prompt: { role: 'Request Engineer', task: 'Implement request builder', context: args, instructions: ['1. Build request objects', '2. Handle headers', '3. Handle body types', '4. Support query params'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const connectionPoolingTask = defineTask('connection-pooling', (args, taskCtx) => ({
  ...
  agent: { name: 'pool-engineer', prompt: { role: 'Connection Pool Engineer', task: 'Implement connection pooling', context: args, instructions: ['1. Pool connections per host', '2. Manage pool sizes', '3. Handle keepalive', '4. Clean stale connections'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const timeoutHandlingTask = defineTask('timeout-handling', (args, taskCtx) => ({
  ...
  agent: { name: 'timeout-engineer', prompt: { role: 'Timeout Engineer', task: 'Implement timeout handling', context: args, instructions: ['1. Connection timeout', '2. Read timeout', '3. Write timeout', '4. Total request timeout'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const interceptorsTask = defineTask('interceptors', (args, taskCtx) => ({
  ...
  agent: { name: 'interceptor-engineer', prompt: { role: 'Interceptor Engineer', task: 'Implement interceptors', context: args, instructions: ['1. Request interceptors', '2. Response interceptors', '3. Error interceptors', '4. Middleware chain'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const validationTask = defineTask('validation', (args, taskCtx) => ({
  ...
  agent: { name: 'qa-engineer', prompt: { role: 'QA Engineer', task: 'Validate library', context: args, instructions: ['1. Verify features', '2. Check test coverage', '3. Validate API design', '4. Calculate score'] }, outputSchema: { type: 'object', required: ['overallScore', 'passedChecks', 'artifacts'], properties: { overallScore: { type: 'number' }, passedChecks: { type: 'array' }, artifacts: { type: 'array' } } } },
