...
export const handshakeTask = defineTask('handshake', (args, taskCtx) => ({
  ...
  agent: { name: 'handshake-engineer', prompt: { role: 'WebSocket Engineer', task: 'Implement WebSocket handshake', context: args, instructions: ['1. Parse upgrade request', '2. Validate Sec-WebSocket-Key', '3. Generate accept key', '4. Send handshake response'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const frameParserTask = defineTask('frame-parser', (args, taskCtx) => ({
  ...
  agent: { name: 'frame-engineer', prompt: { role: 'Frame Parser Engineer', task: 'Implement frame parsing', context: args, instructions: ['1. Parse frame header', '2. Handle masking', '3. Parse payload', '4. Validate frames'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const frameWriterTask = defineTask('frame-writer', (args, taskCtx) => ({
  ...
  agent: { name: 'writer-engineer', prompt: { role: 'Frame Writer Engineer', task: 'Implement frame writing', context: args, instructions: ['1. Write frame header', '2. Handle large payloads', '3. Support all opcodes', '4. Optimize writes'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const pingPongTask = defineTask('ping-pong', (args, taskCtx) => ({
  ...
  agent: { name: 'keepalive-engineer', prompt: { role: 'Keep-Alive Engineer', task: 'Implement ping/pong', context: args, instructions: ['1. Send ping frames', '2. Handle pong responses', '3. Detect dead connections', '4. Configure intervals'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const compressionTask = defineTask('compression', (args, taskCtx) => ({
  ...
  agent: { name: 'compression-engineer', prompt: { role: 'Compression Engineer', task: 'Implement permessage-deflate', context: args, instructions: ['1. Negotiate extension', '2. Compress messages', '3. Decompress messages', '4. Handle context takeover'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const connectionMgmtTask = defineTask('connection-mgmt', (args, taskCtx) => ({
  ...
  agent: { name: 'connection-engineer', prompt: { role: 'Connection Engineer', task: 'Implement connection management', context: args, instructions: ['1. Track connections', '2. Handle close frames', '3. Implement graceful shutdown', '4. Add connection limits'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
