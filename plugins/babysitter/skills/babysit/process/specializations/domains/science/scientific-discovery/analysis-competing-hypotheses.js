...
export const achHypothesisGenerationTask = defineTask('ach-hypothesis-generation', (args, taskCtx) => ({
  ...
  agent: { name: 'general-purpose', prompt: { role: 'Intelligence analyst', task: 'Generate comprehensive set of hypotheses', context: args, instructions: ['List all provided hypotheses', 'Generate additional plausible hypotheses', 'Include unlikely but possible hypotheses', 'Ensure hypotheses are mutually exclusive where possible', 'Document hypothesis rationale'], outputFormat: 'JSON with hypotheses, additionalHypotheses, rationale, artifacts' }, outputSchema: { type: 'object', required: ['hypotheses', 'artifacts'], properties: { hypotheses: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, hypothesis: { type: 'string' }, description: { type: 'string' } } } }, additionalHypotheses: { type: 'array' }, rationale: { type: 'string' }, artifacts: { type: 'array' } } } },
...
export const achConsistencyRatingTask = defineTask('ach-consistency-rating', (args, taskCtx) => ({
  ...
  agent: { name: 'general-purpose', prompt: { role: 'Consistency rater', task: 'Rate consistency of each evidence item with each hypothesis', context: args, instructions: ['Rate as: Consistent (C), Inconsistent (I), Neutral (N)', 'Consider: If hypothesis is true, would evidence be expected?', 'Focus on disconfirming evidence', 'Document rating rationale'], outputFormat: 'JSON with ratings, ratingMatrix, rationale, artifacts' }, outputSchema: { type: 'object', required: ['ratings', 'ratingMatrix', 'artifacts'], properties: { ratings: { type: 'array' }, ratingMatrix: { type: 'object' }, rationale: { type: 'object' }, artifacts: { type: 'array' } } } },
  io: { inputJsonPath: `tasks/${taskCtx.effectId}/input.json`, outputJsonPath: `tasks/${taskCtx.effectId}/result.json` }, labels: ['agent', 'ach']
}));
