...
export const architectureTask = defineTask('architecture', (args, taskCtx) => ({
  ...
  agent: { name: 'architect', prompt: { role: 'Systems Architect', task: 'Design transparent proxy', context: args, instructions: ['1. Interception design', '2. Traffic flow', '3. SSL handling', '4. Platform specifics'] }, outputSchema: { type: 'object', required: ['design', 'artifacts'], properties: { design: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const packetRedirectTask = defineTask('packet-redirect', (args, taskCtx) => ({
  ...
  agent: { name: 'redirect-engineer', prompt: { role: 'Redirect Engineer', task: 'Implement packet redirection', context: args, instructions: ['1. iptables rules', '2. TPROXY setup', '3. REDIRECT target', '4. Routing tables'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const originalDestTask = defineTask('original-dest', (args, taskCtx) => ({
  ...
  agent: { name: 'dest-engineer', prompt: { role: 'Destination Engineer', task: 'Get original destination', context: args, instructions: ['1. SO_ORIGINAL_DST', '2. IP_TRANSPARENT', '3. getsockopt handling', '4. NAT table lookup'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const sslInterceptTask = defineTask('ssl-intercept', (args, taskCtx) => ({
  ...
  agent: { name: 'ssl-engineer', prompt: { role: 'SSL Intercept Engineer', task: 'Implement SSL interception', context: args, instructions: ['1. MITM setup', '2. Dynamic cert generation', '3. SNI extraction', '4. HSTS handling'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const caManagementTask = defineTask('ca-management', (args, taskCtx) => ({
  ...
  agent: { name: 'ca-engineer', prompt: { role: 'CA Engineer', task: 'Implement CA management', context: args, instructions: ['1. Root CA generation', '2. Cert signing', '3. Cert caching', '4. Client distribution'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const trafficAnalysisTask = defineTask('traffic-analysis', (args, taskCtx) => ({
  ...
  agent: { name: 'analysis-engineer', prompt: { role: 'Traffic Analysis Engineer', task: 'Implement traffic analysis', context: args, instructions: ['1. Protocol detection', '2. Content inspection', '3. Metadata extraction', '4. Threat detection'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const policyEngineTask = defineTask('policy-engine', (args, taskCtx) => ({
  ...
  agent: { name: 'policy-engineer', prompt: { role: 'Policy Engineer', task: 'Implement policy engine', context: args, instructions: ['1. Rule definition', '2. Policy evaluation', '3. Action execution', '4. Audit logging'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const loggingTask = defineTask('logging', (args, taskCtx) => ({
  ...
  agent: { name: 'logging-engineer', prompt: { role: 'Logging Engineer', task: 'Implement logging', context: args, instructions: ['1. Connection logs', '2. Traffic logs', '3. Policy logs', '4. Performance metrics'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const validationTask = defineTask('validation', (args, taskCtx) => ({
  ...
  agent: { name: 'qa-engineer', prompt: { role: 'QA Engineer', task: 'Validate proxy', context: args, instructions: ['1. Verify interception', '2. Check SSL', '3. Validate tests', '4. Calculate score'] }, outputSchema: { type: 'object', required: ['overallScore', 'passedChecks', 'artifacts'], properties: { overallScore: { type: 'number' }, passedChecks: { type: 'array' }, artifacts: { type: 'array' } } } },
