...
export const architectureTask = defineTask('architecture', (args, taskCtx) => ({
  ...
  agent: { name: 'architect', prompt: { role: 'Systems Architect', task: 'Design health check architecture', context: args, instructions: ['1. Probe architecture', '2. Scheduler design', '3. State management', '4. Distributed coordination'] }, outputSchema: { type: 'object', required: ['design', 'artifacts'], properties: { design: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const probeFrameworkTask = defineTask('probe-framework', (args, taskCtx) => ({
  ...
  agent: { name: 'framework-engineer', prompt: { role: 'Framework Engineer', task: 'Build probe framework', context: args, instructions: ['1. Probe interface', '2. Result handling', '3. Timeout management', '4. Retry logic'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const tcpProbeTask = defineTask('tcp-probe', (args, taskCtx) => ({
  ...
  agent: { name: 'tcp-engineer', prompt: { role: 'TCP Probe Engineer', task: 'Implement TCP probe', context: args, instructions: ['1. Connection check', '2. Port scanning', '3. Latency measurement', '4. TLS validation'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const grpcProbeTask = defineTask('grpc-probe', (args, taskCtx) => ({
  ...
  agent: { name: 'grpc-engineer', prompt: { role: 'gRPC Probe Engineer', task: 'Implement gRPC probe', context: args, instructions: ['1. Health check RPC', '2. Reflection support', '3. TLS handling', '4. Metadata validation'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const dashboardTask = defineTask('dashboard', (args, taskCtx) => ({
  ...
  agent: { name: 'dashboard-engineer', prompt: { role: 'Dashboard Engineer', task: 'Build status dashboard', context: args, instructions: ['1. Service status view', '2. Historical graphs', '3. Incident timeline', '4. API endpoints'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
