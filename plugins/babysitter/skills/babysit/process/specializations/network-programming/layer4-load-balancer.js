...
export const architectureTask = defineTask('architecture', (args, taskCtx) => ({
  ...
  agent: { name: 'architect', prompt: { role: 'Systems Architect', task: 'Design L4 LB architecture', context: args, instructions: ['1. NAT vs DSR design', '2. Connection tracking', '3. Data plane design', '4. Control plane design'] }, outputSchema: { type: 'object', required: ['design', 'artifacts'], properties: { design: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const connectionHandlingTask = defineTask('connection-handling', (args, taskCtx) => ({
  ...
  agent: { name: 'connection-engineer', prompt: { role: 'Connection Engineer', task: 'Implement connection handling', context: args, instructions: ['1. Accept connections', '2. Forward to backend', '3. NAT translation', '4. Connection tracking'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const algorithmsTask = defineTask('algorithms', (args, taskCtx) => ({
  ...
  agent: { name: 'algorithm-engineer', prompt: { role: 'Algorithm Engineer', task: 'Implement LB algorithms', context: args, instructions: ['1. Round robin', '2. Least connections', '3. IP hash', '4. Weighted algorithms'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const healthCheckingTask = defineTask('health-checking', (args, taskCtx) => ({
  ...
  agent: { name: 'health-engineer', prompt: { role: 'Health Check Engineer', task: 'Implement health checks', context: args, instructions: ['1. TCP health check', '2. HTTP health check', '3. Check intervals', '4. Failure thresholds'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const sessionPersistenceTask = defineTask('session-persistence', (args, taskCtx) => ({
  ...
  agent: { name: 'session-engineer', prompt: { role: 'Session Engineer', task: 'Implement session persistence', context: args, instructions: ['1. Source IP persistence', '2. Persistence timeout', '3. Shared state', '4. Failover handling'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const haModeTask = defineTask('ha-mode', (args, taskCtx) => ({
  ...
  agent: { name: 'ha-engineer', prompt: { role: 'HA Engineer', task: 'Implement high availability', context: args, instructions: ['1. Active/passive mode', '2. Health monitoring', '3. Failover mechanism', '4. State synchronization'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const validationTask = defineTask('validation', (args, taskCtx) => ({
  ...
  agent: { name: 'qa-engineer', prompt: { role: 'QA Engineer', task: 'Validate load balancer', context: args, instructions: ['1. Verify algorithms', '2. Check failover', '3. Validate tests', '4. Calculate score'] }, outputSchema: { type: 'object', required: ['overallScore', 'passedChecks', 'artifacts'], properties: { overallScore: { type: 'number' }, passedChecks: { type: 'array' }, artifacts: { type: 'array' } } } },
