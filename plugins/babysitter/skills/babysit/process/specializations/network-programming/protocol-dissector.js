...
export const parserImplementationTask = defineTask('parser-implementation', (args, taskCtx) => ({
  ...
  agent: { name: 'parser-engineer', prompt: { role: 'Parser Engineer', task: 'Implement protocol parser', context: args, instructions: ['1. Parse packet header', '2. Handle variable fields', '3. Support nested structures', '4. Handle errors'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const fieldExtractionTask = defineTask('field-extraction', (args, taskCtx) => ({
  ...
  agent: { name: 'extraction-engineer', prompt: { role: 'Extraction Engineer', task: 'Extract field values', context: args, instructions: ['1. Extract primitive types', '2. Handle endianness', '3. Extract strings', '4. Handle arrays'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const displayTask = defineTask('display', (args, taskCtx) => ({
  ...
  agent: { name: 'display-engineer', prompt: { role: 'Display Engineer', task: 'Implement display formatting', context: args, instructions: ['1. Tree structure', '2. Field formatting', '3. Enum display', '4. Hex dump'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const wiresharkTask = defineTask('wireshark', (args, taskCtx) => ({
  ...
  agent: { name: 'wireshark-engineer', prompt: { role: 'Wireshark Engineer', task: 'Integrate with Wireshark', context: args, instructions: ['1. Plugin structure', '2. Registration', '3. Preference handling', '4. Build system'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const validationFinalTask = defineTask('validation-final', (args, taskCtx) => ({
  ...
  agent: { name: 'qa-engineer', prompt: { role: 'QA Engineer', task: 'Validate dissector', context: args, instructions: ['1. Verify parsing', '2. Check display', '3. Validate tests', '4. Calculate score'] }, outputSchema: { type: 'object', required: ['overallScore', 'passedChecks', 'artifacts'], properties: { overallScore: { type: 'number' }, passedChecks: { type: 'array' }, artifacts: { type: 'array' } } } },
  io: { inputJsonPath: `tasks/${taskCtx.effectId}/input.json`, outputJsonPath: `tasks/${taskCtx.effectId}/result.json` }, labels: ['network', 'dissector', 'validation']
}));
