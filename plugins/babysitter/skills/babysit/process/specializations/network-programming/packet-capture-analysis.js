...
export const pcapInterfaceTask = defineTask('pcap-interface', (args, taskCtx) => ({
  ...
  agent: { name: 'pcap-engineer', prompt: { role: 'Pcap Engineer', task: 'Interface with libpcap', context: args, instructions: ['1. Initialize pcap', '2. List interfaces', '3. Open capture', '4. Handle permissions'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const packetCaptureTask = defineTask('packet-capture', (args, taskCtx) => ({
  ...
  agent: { name: 'capture-engineer', prompt: { role: 'Capture Engineer', task: 'Implement packet capture', context: args, instructions: ['1. Capture packets', '2. Handle timeouts', '3. Buffer management', '4. Error handling'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const protocolDecodingTask = defineTask('protocol-decoding', (args, taskCtx) => ({
  ...
  agent: { name: 'decoder-engineer', prompt: { role: 'Protocol Decoder Engineer', task: 'Implement protocol decoding', context: args, instructions: ['1. Decode Ethernet', '2. Decode IP/IPv6', '3. Decode TCP/UDP', '4. Decode application protocols'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const statisticsTask = defineTask('statistics', (args, taskCtx) => ({
  ...
  agent: { name: 'stats-engineer', prompt: { role: 'Statistics Engineer', task: 'Collect traffic statistics', context: args, instructions: ['1. Packet counts', '2. Byte counts', '3. Protocol distribution', '4. Flow tracking'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const exportTask = defineTask('export', (args, taskCtx) => ({
  ...
  agent: { name: 'export-engineer', prompt: { role: 'Export Engineer', task: 'Implement export formats', context: args, instructions: ['1. PCAP export', '2. PCAPNG export', '3. JSON export', '4. CSV export'] }, outputSchema: { type: 'object', required: ['implementation', 'artifacts'], properties: { implementation: { type: 'object' }, artifacts: { type: 'array' } } } },
...
export const validationTask = defineTask('validation', (args, taskCtx) => ({
  ...
  agent: { name: 'qa-engineer', prompt: { role: 'QA Engineer', task: 'Validate tool', context: args, instructions: ['1. Verify features', '2. Check protocols', '3. Validate tests', '4. Calculate score'] }, outputSchema: { type: 'object', required: ['overallScore', 'passedChecks', 'artifacts'], properties: { overallScore: { type: 'number' }, passedChecks: { type: 'array' }, artifacts: { type: 'array' } } } },
